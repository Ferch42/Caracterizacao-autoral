...
def Output_string(cls,spmd,md,clf):
	return 'C:\\Users\\Fernando\\Desktop\\DEEP LEARNING 2\\Resultados\\Embeddings (stopwords)\\'+clf+'\\'+spmd+'\\'+md+'\\'+cls+'_'+clf+'_'+spmd+'_'+md+'.txt'

def Classify(cls,clf,spmd,md):

	#Start moment 
	Start_moment=time.time()
	title='Classifying '+cls+' using W2Vec '+clf+'-'+spmd+'_'+md+' without stopwords'
	print(title)

	#Loading model
	print('Loading model',spmd,md)
	model = KeyedVectors.load_word2vec_format(Model_filename(spmd,md), unicode_errors="ignore")
...
### The main
supermodels=['cbow','skip']
models=['s50','s100','s300','s600','s1000']
Classifiers=['LogReg','KNN']

for s in supermodels:
	for m in models:
		for c in Classifiers:
			Classify(metadata,c,s,m)
